...
<div class="content_table">
    <table cellspacing="0" cellpadding="0" border="0" width="100%">
        <tr>
            ...
            <th class="th_no_cursor" width="31"><input type="checkbox" class="custom_chk" id="selectAllItems"
                                                       onclick="selectAllItems(<?= count($result) ?>)"/></th>
            <th width="150" class="th_left th_no_cursor">Image</th>
            <th class="th_left" onclick="sort('title')">
                <div id="title" class="sort icon_no_sort">Title</div>
            </th>

            <th class="th_left" onclick="sort('type')">
                <div id="title" class="sort icon_no_sort">Danh mục</div>
            </th>

            <th width="70" onclick="sort('status')">
                <div id="status" class="sort icon_no_sort">Status</div>
            </th>
        ...
        if ($result) {
            ...
            foreach ($result as $k => $v) {
                ...
                <tr class="item_row<?= $i ?> <?php ($k % 2 == 0) ? print 'row1' : print 'row2' ?>">
                    ...
                    <td class="td_no_padd"><input type="checkbox" class="custom_chk" id="item<?= $i ?>"
                                                  onclick="selectItem(<?= $i ?>)" value="<?= $v->id ?>"/></td>
                    <td class="td_center"><a href="<?=PATH_URL.'admincp/'.$module.'/update/'.$v->id?>"><img class="img_block" alt="" src="<?php  echo getCacheImageByUrl(PATH_URL."static/".$v->image,150, 80,1); ?>" /></a></td>
                    </td>
                    <td class="th_left"><a
                            href="<?= PATH_URL . 'admincp/' . $module . '/update/' . $v->id ?>"><?= $v->title ?></a>
                    </td>
                    <td class="th_left">
                            <span><?php if(isset($category[$v->type]))if(isset($v->type)) echo  $category[$v->type]->name; ?></span>
