...
function showRequest(formData, jqForm, options) {
	var form = jqForm[0];
	if(form.titleAdmincp.value == '' || form.descriptionAdmincp.value == '' || $('#contentAdmincp').val() == '<br>' || $('#contentAdmincp').val() == ''){
			...
			<tr>
				...
				<td class="right_text_field">
					<select name="typeAdmincp" id="typeAdmincp">
						<option <?php if(isset($result->type)) if($result->type == 'Tin tức') print "selected=\"selected\"";?> value="Tin mới">Tin mới</option>
						<option <?php  if(isset($result->type)) if($result->type == 'Cập nhật') print "selected=\"selected\"";?> value="Cập nhật">Cập nhật</option>
						<option <?php  if(isset($result->type)) if($result->type == 'Sự kiện') print "selected=\"selected\"";?> value="Sự kiện">Sự kiện</option>
					</select>
				</td>
			</tr>
		</table>
	...
	<div class="row_text_field">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
			<tr>
				...
				<td class="right_text_field"><textarea name="descriptionAdmincp" cols="" rows="8"><?php if(isset($result->description)) { print $result->description; }else{ print '';} ?></textarea></td>
			</tr>
		</table>
	</div>
