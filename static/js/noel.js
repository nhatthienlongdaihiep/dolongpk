...
$(document).ready(function() {
  ...
  if(!user_login){
    ...
    $('.plugin, .mail, .sms, .ev_noel, .maria').click(function(e) {
      e.preventDefault();
      $('.btn_dangky').trigger('click');
    });
  }else{
    $('.plugin, .mail, .sms').click(function(e) {
        e.preventDefault();
        var h = $( window ).height();
        var top = h/2-200;
        setTimeout(function(){
    ...
    $('.maria').click(function(e) {
        e.preventDefault();
        $.fancybox({
          type: 'ajax',
          href: root+'gift/giftMaria'
        })
    });
    if(uri == '/' || uri == '/trang-chu'){
      var h = $( window ).height();
      var top = h/2-200;
      setTimeout(function(){
        var point = $('.header').offset();
